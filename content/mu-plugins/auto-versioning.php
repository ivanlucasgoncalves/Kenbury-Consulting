<?php
/*
 * Plugin Name: Auto-Versioning
 * Description: Script & style auto-versioning provider plugin
 * Version:     1.0
...
if ( ! class_exists( 'AutoVersioning' ) ) {
	...
	class AutoVersioning {
		 ...
		 */
		public static function auto_version( $url, $add_ver_to_filename = false ) {

			$version = false;
			$filename = self::guess_filename_from_url( $url );

			// Dont want it doing url tricks on dev
			if ( defined('WP_ENV') && WP_ENV == 'local' ) {
                $add_ver_to_filename = false;
			}
		...
		private static function guess_filename_from_url( $url ) {
			...
			$url_mappings = array(
				...
				site_url( '/' )                               => ABSPATH,
				home_url( '/' )                               => dirname( ABSPATH )  . '/', // NOTE this assumes that wordpress is in a separate directory (i.e. wp/)
			);

			$filename = null;$found = false;
